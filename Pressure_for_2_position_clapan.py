...
# крутой рост на малых ходах, насыщение к полному подъему
# def rho_flow(hdc):
#     return np.interp(hdc, [0,.05,.10,.15,.20,.25,.30,.40],
#                           [1.0,1.6,2.0,2.2,2.32,2.4,2.45,2.5])
...
# Пружина: двухпозиционный режим (рис. 40-42: линия потока выше
# линии пружины на подъеме) -> c мала, dF_p/dh > c на малых h
c_spring = 2200.0
# Предварительный натяг из условия открытия при P_нач, (III.42)
...
time=[0.0]; press=[P_sat(T)]; lift=[0.0]; flow=[0.0]
y = np.array([n_g, n_l, 0.0, 0.0])   # [ng, nl, h, v]

# ---------- Правые части: баланс масс + (III.56) ----------
def deriv(y, T_, P_):
    ng, nl, hh, vv = y
    Gv  = G_valve(P_, T_, hh, F_c, d_c)
    dng = Q_dot/Lv(T_) - Gv/M
    dnl = -Q_dot/Lv(T_)
    F_p = F_c*(P_ - P_atm)*rho_flow(max(hh,0)/d_c)          # (III.46)-(III.52)
    a   = (F_p - c_spring*(hh + h_0) - eta*vv - m*g)/m      # (III.56)
...
for i in range(n_steps):
    ...
    if y[2] < 0:      y[2] = 0.0; y[3] = max(y[3], 0.0)   # седло
    if y[2] > h_stop: y[2] = h_stop; y[3] = 0.0           # ограничитель
    if y[1] <= 0:
        print(f"Жидкость испарилась при t = {i*dt:.1f} с"); break

    # запись после шага с уточненным P
    T = solve_T(y[0], y[1], T)
    P = P_sat(T)
    time.append(i*dt); press.append(P)
    lift.append(y[2]); flow.append(G_valve(P, T, y[2], F_c, d_c))

print(f"c пружины = {c_spring:.0f} Н/м, h0 = {h_0*1000:.2f} мм")
print(f"P макс = {max(press)/1e5:.3f} бар, P мин после открытия = {min(press[int(10/dt):])/1e5:.3f} бар")
...
plt.tight_layout(); plt.savefig('ln2_valve_pop.png', dpi=150); plt.show()